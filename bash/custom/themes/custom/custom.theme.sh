...
# TODO: venv
PS1+="[${_omb_prompt_bold_green}\u@\h:${_omb_prompt_bold_blue}\W${_omb_prompt_normal}]"
PS1+="\$(scm_prompt_info)"
PS1+="\$ "
...
function _omb_theme_prompt_arrow {
    if [ $? -ne 0 ]; then
        echo "${_omb_term_bold_red}➜${_omb_term_normal}"
    else
        echo "${_omb_term_bold_green}➜${_omb_term_normal}"
    fi
}
